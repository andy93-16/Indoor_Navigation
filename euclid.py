...
class Geometry:
    ...
    def distance(self, other):
        ...
        if c:
            return c.length
        return 0.0
"""This function has been implemented to make the intersect between point2 
and LineSegment2 
"""
def _intersect_point2_line2(P, L):
    M=Matrix3()
    M.a=L.p1.x
    M.b=L.p1.y
    M.c=float(1)
    ...
    M.j=P.y
    M.k=float(1)
    if M.determinant()/2 == 0:
        #if M.determinant/2>0 point is on left of the LineSegment
        #M.determinant/2 is the area of triangle L.p1, L.p2 and P
       return True
    else:
       return False
...
class Point2(Vector2, Geometry):
    ...
    def _intersect_line2(self, other):
        return _intersect_point2_line2(self, other)
...
class LineSegment2(Line2):
    ...
    def _intersect_point2(self, other):
        if _intersect_point2_line2(other,self):
            dp1_p2 = abs(self.p1-self.p2)
            if abs(self.p1 - other) < dp1_p2 and abs(self.p2 - other) < dp1_p2:
               return True
            else:
               return False
        else:
