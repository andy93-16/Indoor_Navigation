...
from buffer import Buffer_LineSegment2

if __name__ == "__main__":

    def buffer_from_linesegment(line, buffer_holes=None, dist=0.15, door_size=0.3):
            ...
            for poly in room:
                for side_p1, side_p2, next_side_p2 in zip(poly, poly[1:] + poly[:1], poly[2:] + poly[:2]):
                    ...
                    if not na.x * nb.y > nb.x * na.y:  # check if is left or right angle
                        if angle + 3.14 > 4.85:
                            dict_update[side_p2, rooms.index(room)] = [-side.v.normalize() * c + side_p2, \
                                                                       next_side.v.normalize() * c + side_p2]
        # print(dict_update)
        outies = []
        for room in rooms:
            for poly in room:
    ...
    def visibility_check(first_node, second_node, euc_room):
        ...
        if d > d1:
          ...
          for poly in euc_room:
            for wall in poly:
                if (wall.intersect(first_node) is False and wall.intersect(second_node) is False) and \
                        wall.intersect(LineSegment2(first_node, second_node)) is not None:
                    intsect = True
          if not intsect:
    ...
    def visibility_check_pro(first_node, second_node, euc_room1, euc_room2):
        d = nx.dijkstra_path_length(totalpath, first_node, second_node, weight='weight')
        d1 = abs(first_node - second_node)
        # print(d,d1,nd_room,node_room)
        if d >= d1:
            intsect = False
            for wall in euc_room1:
                if wall.intersect(first_node) is False and wall.intersect(second_node) is False and \
                        wall.intersect(LineSegment2(first_node, second_node)) is not None:
                    intsect = True
            for wall in euc_room2:
                if wall.intersect(first_node) is False and wall.intersect(second_node) is False and \
                        wall.intersect(LineSegment2(first_node, second_node)) is not None:
                    intsect = True
            if not intsect:
    ...
    def are_collinear(wall_1, wall_2):
        l = Line2(wall_1)
        return l.intersect(wall_2.p1) and l.intersect(wall_2.p2)
    ...
    def main():
        ...
        room4 = [[(1, 5), (1, 0), (8, 0), (8, 2),
                  ...
                  (2, 5)]]
        # room4=[[(1, 5),(1, 0),(4,0),(4.2,0.2),(4.2,0),(4.8,0),(5,0.3),(5.2,0),(5.8,0),
        #         (5.8,0.2),(6,0),(8, 0),(8,2),(6, 1),(6,3),(6,5),(4,5),(4,1),(2,2),(2, 3),
        #        (2, 5)]]
        ...
        for node_room in room_list_node:
            for nd_room in room_list_node:
                ...
                if len(room_list_node[nd_room]) == 1 and len(room_list_node[node_room]) == 1 and \
                        room_list_node[node_room][0] == room_list_node[nd_room][0]:

                    visibility_check(node_room, nd_room, euc_rooms[room_list_node[node_room][0]])
                # it check edge intersection of a room_node and door_node
                elif len(room_list_node[node_room]) == 1 and len(room_list_node[nd_room]) == 2:
                     ...
                     if room_list_node[node_room][0] == room_list_node[nd_room][1]:
                        visibility_check(node_room, nd_room, euc_rooms[room_list_node[node_room][0]])
                # it check edge intersection of a door_node and door_node

                elif len(room_list_node[node_room]) == 2 and len(room_list_node[nd_room]) == 2:

                    # it's needed to test the collinearity of two LineSegment to avoid false visibilty

                    if room_list_node[node_room][0] == room_list_node[nd_room][0]:
                        wall_a = find_wall(node_room, euc_rooms[room_list_node[node_room][0]])
                        wall_b = find_wall(nd_room, euc_rooms[room_list_node[nd_room][0]])
                        if not are_collinear(wall_a, wall_b):
                            visibility_check_pro(node_room, nd_room, euc_rooms[room_list_node[node_room][0]], \
                                                 euc_rooms[room_list_node[node_room][1]])

                    if room_list_node[node_room][1] == room_list_node[nd_room][1]:
                        wall_a = find_wall(node_room, euc_rooms[room_list_node[node_room][1]])
                        wall_b = find_wall(nd_room, euc_rooms[room_list_node[nd_room][1]])
                        if not are_collinear(wall_a, wall_b):
                            visibility_check_pro(node_room, nd_room, euc_rooms[room_list_node[node_room][0]], \
                                                 euc_rooms[room_list_node[node_room][1]])
